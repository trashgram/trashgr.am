            ...
            switch (content.text.text) {
                case '/del':
                    if (replyToMessageId !== 0) {
                        const request = { '@type': 'getMessage', chat_id: chatId, message_id: replyToMessageId };
                        TdLibController.send(request).then(result => {
                            const chat = ChatStore.get(chatId);
                            const userId = UserStore.getMyId();
                            EnhancedAction.deleteUserMessages(
                                chatId,
                                chat.type['@type'],
                                userId,
                ...
                case '@admin':
                    if (replyToMessageId !== 0) {
                        await EnhancedAction.tagAdmin(chatId, replyToMessageId, this.props);
                        this.setState({ replyToMessageId: 0 });
                        return;
                    }
                    break;
                case '@everyone':
                    await EnhancedAction.tagEveryone(chatId, replyToMessageId, this.props);
                    this.setState({ replyToMessageId: 0 });
                    return;
            }
        }
